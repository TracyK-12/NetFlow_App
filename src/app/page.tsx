...
export default function MissionsPage() {
  ...
  return (
    /*
      ✅ Correction : `main` retiré ici — il est déjà dans layout.tsx.
         On utilise `div` avec `min-h-screen` + padding responsive.
         `pb-24` : espace en bas pour le mobile header sticky.
    */
    <div className="min-h-screen bg-white p-4 md:p-8 lg:p-12 pb-24">
                ...
                {currentMonthMissions.map((mission) => (
                  <div key={mission.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="overflow-hidden">
                          <p className="font-black text-[#1E293B] truncate">{mission.name}</p>
                          <p className="text-xs text-slate-400 font-medium">{mission.totalHours}h × {mission.grossRate}€</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <div className="text-right">
                          <p className={`font-black text-lg ${mission.status === 'perçu' ? 'text-green-600' : 'text-blue-600'}`}>
                            {mission.netIncome.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
                          </p>
                          <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${mission.status === 'perçu' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                            {mission.status === 'perçu' ? 'Reçu' : 'En cours'}
                          </span>
                        </div>
                    ...
                    {(mission.startDate || mission.endDate) && (
                      <div
                        className="mt-3 pt-3 border-t border-slate-50 flex items-center gap-2 text-xs text-slate-400 cursor-pointer"
